<?php
    ...
    //定义“支付”函数。
    function paymentOrders($Orders)
    {
        //声明“获取订单”数据访问层。
        $Orders_DAL = new OrdersDAL();
        $Orders_DAL->Alter1($Orders);    //调用“修改”方法。
    }

    //定义“确认收货”函数。
    function receiptOrders($Orders)
    {
        //声明“获取订单”数据访问层。
        $Orders_DAL = new OrdersDAL();
        $Orders_DAL->Alter2($Orders);    //调用“修改”方法。
    }
    ...
    if ($_POST['submit'] == '支付')
    {
        ...
        paymentOrders($Orders);    //调用“支付”函数。
        showOrders($Orders);    //调用“获取订单”函数。
    }

    //判断用户是否点击的是“确认收货”按钮。
    if ($_POST['submit'] == '确认收货')
    {
        //为“编号”属性赋值。
        $Orders->id = $_POST['id'][$index];
        receiptOrders($Orders);    //调用“确认收货”函数。
        showOrders($Orders);    //调用“获取订单”函数。
    }
?>